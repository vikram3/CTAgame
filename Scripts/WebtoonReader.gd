...
func _ready():
	...
	# ── Listen for window resize (PC) or screen rotation (mobile) ──
	get_viewport().size_changed.connect(_on_viewport_size_changed)


# ── Resize / rotation handler ────────────────────────────────
func _on_viewport_size_changed():
	# Debounce: don't rebuild on every intermediate resize frame
	_resize_pending = true
	_resize_timer = RESIZE_DEBOUNCE_SEC
...
func _process(delta):
	...
	# Auto-save scroll every 2 seconds
	save_timer += delta
	if save_timer >= 2.0:
		save_timer = 0.0
...
func _rebuild_for_new_size():
	# Save current scroll ratio (position / total height) so we can restore proportionally
	var total_h = panel_container.size.y
	var scroll_ratio = 0.0
	if total_h > 0:
		scroll_ratio = float(scroll_container.scroll_vertical) / total_h
	...
	var new_total_h = panel_container.size.y
	scroll_container.scroll_vertical = int(scroll_ratio * new_total_h)
	print("Rebuilt for new size: ", get_viewport().get_visible_rect().size,
		  " | panel_width: ", ChapterData.get_panel_width())
...
func add_static_panel(entry: ChapterData.PanelEntry, index: int):
	var panel_width = ChapterData.get_panel_width()  # ← dynamic

	var texture_rect = TextureRect.new()
	texture_rect.name = "Panel_" + str(index)
	texture_rect.custom_minimum_size = Vector2(panel_width, 400)
...
func _apply_texture(texture_rect: TextureRect, texture: Texture2D):
	...
	texture_rect.size = Vector2(panel_width, panel_width * aspect)
